...
class LoadImages:  # for inference
    def __init__(self, path, img_size=(1216, 352)):
...
class JointDataset(LoadImagesAndLabels):  # for training
    def __init__(self, root, paths, img_size=(1088,608), augment=False, transforms=None):
        ...
        for ds, path in paths.items():
            ...
            self.label_files[ds] = [x.replace('images', 'labels-vehicle').replace('.png', 
'.txt').replace('.jpg', '.txt')
                                for x in self.img_files[ds]]
        
#        print(self.label_files.items())
